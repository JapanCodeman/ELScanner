...
function Login(props) {
  ...
  useEffect(() => {
    if (userRole === 'Student' && logged_status === 'LOGGED_IN' && window.sessionStorage.getItem('token')) {
      navigate('/home')
    } 
  ...
  async function handleSubmit(e) {
    ...
    let configSet = {
      ...
      .then(response => {
        if (response.data.data === 'CLASS_RESET') {
          window.sessionStorage.setItem('token', response.data.token)
          window.alert('You need to select your new class for this year - please choose carefully!')
